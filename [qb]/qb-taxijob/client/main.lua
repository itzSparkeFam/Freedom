...
function setupTarget()
    CreateThread(function()
        ...
        local coords = vector4(901.34, -170.06, 74.08, 228.81)
        local jobRequired = "taxi" -- Ensure this is set or passed dynamically
        ...
        -- Spawn Ped
        local ped = CreatePed(4, modelHash, coords.x, coords.y, coords.z - 1, coords.w, false, false)
        FreezeEntityPosition(ped, true)
        SetEntityInvincible(ped, true)
        SetBlockingOfNonTemporaryEvents(ped, true)

        -- Assign Scenario
        if 'WORLD_HUMAN_AA_COFFEE' then
            TaskStartScenarioInPlace(ped, 'WORLD_HUMAN_AA_COFFEE', 0, true)
        end

        -- Assign Animation
        if DoesAnimDictExist('abigail_mcs_1_concat-0') then
            RequestAnimDict('abigail_mcs_1_concat-0')
            while not HasAnimDictLoaded('abigail_mcs_1_concat-0') do
                Wait(0)
            end
            TaskPlayAnim(ped, 'abigail_mcs_1_concat-0', 'csb_abigail_dual-0', 8.0, -8.0, -1, 1, 0, false, false, false)
        end
        ...
        exports.ox_target:addLocalEntity(ped, {
            {
                ...
                distance = 2.5,
                groups = jobRequired and { [jobRequired] = 0 } or nil, -- Restrict interaction by job
                canInteract = function(entity, distance)
                    return not IsPedDeadOrDying(entity, true) and not IsPedAPlayer(entity)
                end
            }
        })
    end)
