...
SD.Inventory.RegisterUsableItem(Config.Items.Thermite, function(source) 
	TriggerClientEvent('sd-bobcat:client:startHeist', source) 
end) 

SD.Inventory.RegisterUsableItem(Config.Items.Keycard, function(source)
...
RegisterNetEvent('sd-bobcat:server:removeItem', function(item)
	SD.Inventory.RemoveItem(source, item, 1)
end)

-- Check for items listed in Config.Items
SD.Callback.Register('sd-bobcat:server:hasItem', function(source, item)
...
SD.Callback.Register('sd-bobcat:server:getCops', function(source)
...
SD.Callback.Register('sd-bobcat:server:cooldown', function(source)
    return(cooldown)
end)

-- Callback to add player to bobcatplayers table
SD.Callback.Register('sd-bobcat:server:addPlayerCallback', function(source)
	local added = false
    local src = source
...
RegisterNetEvent('sd-bobcat:giveRandomBox', function(box)
    ...
    if cooldown and distances[box] <= distanceThreshold and not lootedBoxes[box] then
        ...
        for i = 1, math.random(boxConfig.Amount[1], boxConfig.Amount[2]) do
            ...
            SD.Inventory.AddItem(source, randomItem, amount)
        end
        Wait(300)
    end
end)
...
RegisterNetEvent('sd-bobcat:server:particles', function(ptfxCoords)
    TriggerClientEvent('sd-bobcat:client:particles', -1, ptfxCoords)
end)
...
RegisterNetEvent('sd-bobcat:server:resetVault', function()
    ...
    RemoveGuards()
        ...
        -- Lock the doors
        for _, doorData in ipairs(doors) do SD.Doorlock.UpdateState(doorData) end -- Lock all doors 
...
SD.CheckVersion('sd-versions/sd-bobcat') -- Check version of specified resource