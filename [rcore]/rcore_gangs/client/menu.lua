    ...
    if menu == 'SUBMENU_TERRITORY' then
        if zone and zone.gangPresence then
            for index, gang in ipairs(zone.gangPresence) do
                ...
                if Gang and gang.name == Gang.name then
                    if Menu.Button('GIVE_UP_TERRITORY', Locale.MENU_GIVE_UP_TERRITORY, "", true) then
                        Menu.CloseMenu()
                        TriggerServerEvent("rcore_gangs:server:giveTerritoryUp")
                    end
                end
            end
        else
...
AddEventHandler('rcore_gangs:client:set_gangs', function(gangs)
    ...
    while Menu.CurrentMenu() do
        ...
        if menu == 'SUBAMENU_GARAGE' then
            ...
            if gang.vehicles then
                ...
                for index, vehicle in ipairs(gang.vehicles) do
                    local id = vehicle.model .. '.' .. vehicle.color .. '_' .. id .. ((vehicle.plate and ':' .. vehicle.plate) or "") 
        ...
        if menu == 'SUBAMENU_GARAGE_INSERT' then
            ...
            local id = tonumber(string.match(value, "_([0-9]+)"))
            local gang = gangs[id]
            ...
            if Menu.Button('ACCEPT', Locale.MENU_BUTTON_CONFIRM, nil, true) then
                local vehiclePlate = GetVehicleNumberPlateText(ShowcaseVehicle)
                ...
                TriggerServerEvent('rcore_gangs:server:purchase_vehicle', vehicleModel, vehicleColor, vehiclePlate, id)
        ...
        if menu == 'SUBAMENU_GARAGE_REMOVE' then
            ...
            if DoesEntityExist(ShowcaseVehicle) then
                ...
                ShowcaseVehicle = CreateShowcaseVehicle(model, color, plate)
            end
